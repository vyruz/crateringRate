...
grid = [[EMPTY for x in range(SIZE)] for x in range(SIZE)]
...
def makeImpact(grid):
    ...
    for i in range(int((craterSize/2))+2):
        for j in range(int(((craterSize//2)+2)-i/2)):
    ...
    for i in range(int((craterSize/2)+1)):
        for j in range(int(((craterSize/2)+1)-i/2)):
            if(imp_i+i >= SIZE):
                if(imp_j+j >= SIZE):
